/*	
    ...
    Hardware:		LEDs en el puerto A, 2 pb en puerto b, 5 display de 7
			segmentos multiplexados en C con su selector en D
...
INC_CONT:		     ; INCREMENTO CONTADOR
    INCF    PORTA
    INCF    VAR
    
    RETURN
    
DEC_CONT:		     ; DECREMENTO CONTADOR
    DECF    PORTA
    DECF    VAR
    
    RETURN
    
CONT_TMR0:		     ; Actualización de displays
    RESET_TMR0		     ; Reinicio TMR0
    BCF	      PORTD,0        ; Limpiamos selector DEC
    BCF	      PORTD,1
    BCF	      PORTD,2	     
    BCF	      PORTD,3	     ; Limpiamos selector HEX
    BCF	      PORTD,4
    
    
    
    BTFSC     BANDERAS, 4   ; Se chequea bit 4 para actualizar centenas
    GOTO      DISP_CENT
    BTFSC     BANDERAS, 3   ; Se chequea bit 3 para actualizar decenas
    GOTO      DISP_DEC
    BTFSC     BANDERAS, 2   ; Se chequea bit 2 para actualizar unidades
    GOTO      DISP_UNI
    BTFSC     BANDERAS, 1   ; Se chequea bit 1 para actualizar bit más significativos de Hexa
    GOTO      DISP_PRINC_H
    BTFSC     BANDERAS, 0   ; Se chequea bit 0 para actualizar bit menos significativos de Hexa
    GOTO      DISP_SEC_H
  
DISP_SEC_H:
    MOVF    DISPLAY_VAR, W ; Movemos valor de disp principal a w
    MOVWF   PORTC	     ; Valor de la tabla a puerto
    BSF	    PORTD, 0	     ; Se enciende display 6
    
    BCF	    BANDERAS, 0	     ; Se modifican los bits de bandera para actualizar secuencialmente displays
    BSF	    BANDERAS, 1
        
    RETURN
    
DISP_PRINC_H:
    MOVF      DISPLAY_VAR+1, W ; Movemos valor de disp principal hexa a w
    MOVWF     PORTC	       ; Valor de la tabla a puerto
    BSF	      PORTD, 1	       ; Se enciende display 5
    
    BCF	      BANDERAS, 1      ; Se modifican los bits de bandera para actualizar secuencialmente displays
    BSF	      BANDERAS, 2
        
    RETURN

DISP_UNI:
    MOVF      DISPLAY_DEC, W   ; Movemos valor de disp UNIDADES a w
    MOVWF     PORTC	       ; Valor de la tabla a puerto
    BSF	      PORTD, 2	       ; Se enciende display UNIDADES
    
    BCF	      BANDERAS, 2      ; Se modifican los bits de bandera para actualizar secuencialmente displays
    BSF	      BANDERAS, 3
    
    RETURN

DISP_DEC:
    MOVF      DISPLAY_DEC+1, W ; Movemos valor de disp DECENAS a w
    MOVWF     PORTC	       ; Valor de la tabla a puerto
    BSF	      PORTD, 3	       ; Se enciende display DECENAS
    
    BCF	      BANDERAS, 3      ; Se modifican los bits de bandera para actualizar secuencialmente displays
    BSF	      BANDERAS, 4
    
    RETURN
    
DISP_CENT:
    MOVF      DISPLAY_DEC+2, W ; Movemos valor de disp CENTENAS a w
    MOVWF     PORTC	       ; Valor de la tabla a puerto
    BSF	      PORTD, 4	       ; Se enciende display CENTENAS
    
    BCF	      BANDERAS, 4      ; Se modifican los bits de bandera para actualizar secuencialmente displays
    BSF	      BANDERAS, 0
...
; Configuración de lectura de cambios en puerto B
    ...
    BANKSEL PORTA
    CLRF    VAL_UNI               ; Limpieza de variables
    CLRF    VAL_DEC
    CLRF    VAL_CENT
...
LOOP:  
    ...
    CALL    SEPARACION_NIBBLES   ; Subrutina para obtención del valor de contador HEXADECIMAL
    ...
    CALL    CENTENAS		 ; Subrutinas para separación en Centenas, Decenas y Unidades
    CALL    DECENAS
    CALL    UNIDADES
...
CONFIG_PINES:
    ...
    CLRF    TRISC	      ; PORTC completo como output
    BCF	    TRISD, 0	      ; PORTD 0-4 como output
    BCF	    TRISD, 1
    BCF	    TRISD, 2
    BCF	    TRISD, 3
    BCF	    TRISD, 4
...
CENTENAS:
    MOVLW   100	          ; Resta para sacar centenas
    SUBWF   NUM_DEC, F    ; Resultado se almacena de nuevo en var
    INCF    VAL_CENT	  ; +1 centena
    BTFSC   STATUS, 0     ; Verificamos si hubo borrow
    GOTO    CENTENAS	  ; Si lo hubo se sigue restando
    DECF    VAL_CENT      ; Se decrementa valor para centenas pues ya se excedió
    MOVLW   100		  
    ADDWF   NUM_DEC, F	  ; Se regresa la variable
    
    RETURN

DECENAS:
    MOVLW   10	          ; Resta para sacar decenas
    SUBWF   NUM_DEC, F    ; Resultado se almacena de nuevo en var
    INCF    VAL_DEC	  ; +1 decena
    BTFSC   STATUS, 0     ; Verificamos si si hubo borrow
    GOTO    DECENAS	  ; Si lo hubo se sigue restando
    DECF    VAL_DEC	  ; Se decremento valor para DECENAS
    MOVLW   10	
    ADDWF   NUM_DEC, F
...
UNIDADES:
    MOVLW   1	          ; Resta para sacar unidades
    SUBWF   NUM_DEC, F    ; Resultado se almacena de nuevo en var
    INCF    VAL_UNI
    BTFSC   STATUS, 0     ; Verificamos si hubo borrow
    RETURN
    DECF    VAL_UNI	  ; Se decrementa valor para UNIDADES
    MOVLW   1
    ADDWF   NUM_DEC
...
CONFIG_DISPLAY:               ; Subrutina para traducción de valores a 7 seg
    ...
    MOVF    VAL_UNI, W	      ; Valor unidades a W
    CALL    TABLA	      ; Valor a tabla
    MOVWF   DISPLAY_DEC     ; Config 7 segmentos a primer byte de la variable
    
    MOVF    VAL_DEC, W	      ; Valor decebas a W
    CALL    TABLA	      ; Valor a tabla
    MOVWF   DISPLAY_DEC+1    ; Config 7 segmentos a segundo byte de la variable
    
    MOVF    VAL_CENT, W	      ; Valor centenas a W
    CALL    TABLA	      ; Valor a tabla
    MOVWF   DISPLAY_DEC+2     ; Config 7 segmentos a tercer byte de la variable
